...
2. Build the program if necessary.
   a. $ cd filesafe-server
   b. $ cargo build     # This will create a binary file "filesafe-server" in "filesafe-server/target/debug/". Move file as necessary
3. Run the setup utility
   a. $ ./setup.py
4. Edit the configuration file if necessary
   a. $ vi ~/.config/filesafe/filesafe.ini
